...
let subscribeCardToggles = document.querySelectorAll(".subscribe-card__title")

subscribeCardToggles.forEach((toggle)=>{
    toggle.onclick = function () {
        let parent = toggle.closest(".subscribe-card")
        let hiddenBlock = parent.children[1]
        let hiddenChild = hiddenBlock.children[0]
        let hiddenBlockHeight = 0

        if(parent.classList.contains("subscribe-card--active")){
            parent.classList.remove("subscribe-card--active")
            hiddenBlock.style.overflow = "hidden";
        } else{
            parent.classList.add("subscribe-card--active")
            hiddenBlockHeight = hiddenChild.getBoundingClientRect().height
            setTimeout(() => {
                hiddenBlock.style.overflow = "unset";
            }, 500)
        }

        hiddenBlock.style.height = `${hiddenBlockHeight}px`
    }
})
